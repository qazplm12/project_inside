...
public class EmailController {
    ...
    public ResponseEntity<?> registerPerson(@RequestBody PersonRequest personRequest) {
        try {
            // 회원 정보, 회원 비밀번호 null 이면 에러 발생
            ...
            PersonEntity registerPerson = personService.create(person);
            // DTO에도 저장
            PersonRequest responsePersonDTO = PersonRequest.builder()
                    .personId(registerPerson.getPersonId())
                    .personPassword(registerPerson.getPersonPassword())
                    .personNickName(registerPerson.getPersonNickName())
                    .build();

            return ResponseEntity.ok().body(responsePersonDTO);
        }
        catch (Exception e) {
            // 유저 정보는 항상 하나이므로 리스트로 만들어야 하는 ResponseDTO를 사용하지 않고 그냥 PersonDTO 리턴.(무슨 말이지?)
